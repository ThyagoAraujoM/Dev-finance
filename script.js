...
const WalletModal = {
   ...
   close() {
      // Fecha o modal e limpa os valores nele inseridos.
      document
         .querySelector(".js-wallets-overlay")
...
const helpModal = {
   ...
   iconsOff(number) {
      ...
      switch (number) {
         ...
         case 5:
            document
               .querySelector(".js-transactions-text")
               .classList.toggle("on");
            break;
         case 6:
...
function drawChart() {
   ...
   for (let n = 0; n <= months.length - 1; n++) {
      ...
      let amountFixed = Utils.unFormartCurrency(
         Wallet.total(Storage.getFixed())
      );
...
const Graphic = {
   ...
   close() {
      document
         .querySelector(".js-graphic-overlay")
         .classList.remove("is-active");
   },
   closeOut() {
      // Fecha se clicar fora do moda.
   ...
   updateLastMonth() {
      ...
      for (let n = 0; n < months.length - 1; n++) {
         if (actualMonth == months[n]) {
            let name = walletName;
            document.querySelector(".js-past-month-text").innerHTML =
               months[n - 1];
            month =
               JSON.parse(
                  localStorage.getItem(
